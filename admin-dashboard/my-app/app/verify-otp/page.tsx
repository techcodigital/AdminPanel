...
function VerifyOtpContent() {
  ...
  const email = searchParams.get("email") || (typeof window !== "undefined" ? localStorage.getItem("admin_email") : "") || "";
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#080c18", fontFamily:"'Sora',sans-serif", position:"relative", overflow:"hidden", padding:"2rem" }}>
      ...
      <div style={{ position:"relative", zIndex:2, width:"100%", maxWidth:400, background:"rgba(15,21,40,0.97)", border:"0.5px solid rgba(255,255,255,0.08)", borderRadius:20, padding:"2.5rem 2rem", textAlign:"center" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:"2rem", justifyContent:"center" }}>
          ...
          <span style={{ fontSize:10, fontWeight:500, color:"#4f6ef7", background:"rgba(79,110,247,0.12)", border:"0.5px solid rgba(79,110,247,0.3)", padding:"2px 8px", borderRadius:20 }}>Admin Portal</span>
        </div>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"center", marginBottom:"1.5rem" }} onPaste={handlePaste}>
          {otp.map((digit, i) => (
            <input key={i} ref={(el) => { inputRefs.current[i] = el; }} className="otp-input" type="text" inputMode="numeric" maxLength={1} value={digit} placeholder="·" onChange={(e) => handleChange(i, e.target.value)} onKeyDown={(e) => handleKeyDown(i, e)} />
          ))}
        </div>
        ...
        <p style={{ fontSize:13, color:"rgba(255,255,255,0.35)", marginTop:"1.25rem" }}>
          ...
          {timer > 0 ? (
            ...
            <button onClick={handleResend} disabled={resending} style={{ background:"none", border:"none", color:"#4f6ef7", fontWeight:500, cursor:"pointer", fontSize:13, fontFamily:"'Sora',sans-serif", padding:0 }}>
  ...
  return (
    <>
      <style>{`
        ...
        .otp-input { width:48px; height:56px; background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.12); border-radius:12px; font-size:22px; font-weight:600; color:#fff; text-align:center; font-family:'Sora',sans-serif; outline:none; transition:border-color 0.2s,background 0.2s; caret-color:#4f6ef7; }
        .otp-input:focus { border-color:rgba(79,110,247,0.7); background:rgba(79,110,247,0.08); }
        .otp-input:not(:placeholder-shown) { border-color:rgba(79,110,247,0.4); }
        .verify-btn { width:100%; padding:13px; background:linear-gradient(135deg,#4f6ef7,#7c3aed); border:none; border-radius:10px; font-size:14px; font-weight:600; color:#fff; cursor:pointer; font-family:'Sora',sans-serif; transition:opacity 0.2s,transform 0.15s; }
